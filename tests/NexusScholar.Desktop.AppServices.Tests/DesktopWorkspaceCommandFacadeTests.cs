...
public sealed class DesktopWorkspaceCommandFacadeTests
{
    ...
    [TestMethod]
    public void Screening_resolution_confirmation_token_binds_every_authority_and_target_field()
    {
        var preview = new DesktopScreeningResolutionPreview(
            "missing", "workspace", 1, "authority", "sha256:" + new string('1', 64),
            "sha256:" + new string('2', 64), "sha256:" + new string('3', 64),
            "sha256:" + new string('4', 64), "sha256:" + new string('5', 64),
            "sha256:" + new string('6', 64), "sha256:" + new string('7', 64), "conduct",
            "sha256:" + new string('8', 64), "policy", "sha256:" + new string('9', 64),
            "sha256:" + new string('a', 64), "sha256:" + new string('b', 64),
            "sha256:" + new string('c', 64), 0,
            "candidate", "sha256:" + new string('e', 64), "sha256:" + new string('f', 64),
            "correction", "include", "alice", "human", "reviewer", "Eligible.", null, null, null, [],
            FixedTime, "decision", "sha256:" + new string('0', 64),
            ["append decision"], ["human-actor-required"], "operation-token", "pending");
        preview = preview with
        {
            ConfirmationToken =
                DesktopWorkspaceCommandFacade.CreateScreeningResolutionConfirmationToken(preview)
        };
        DesktopScreeningResolutionPreview[] changed =
        [
            preview with { WorkspaceDirectory = "other-workspace" },
            preview with { WorkspaceId = "other-workspace-id" },
            preview with { ExpectedProjectRevision = 2 },
            ...
            preview with { ResultingHeadDigest = null! },
            preview with { PriorEntryCount = 1 },
            preview with { CandidateId = "candidate/2" },
            preview with { TargetDigest = null! },
            preview with { TargetSummaryDigest = null! },
            preview with { DecisionKind = "adjudication" },
            preview with { Verdict = "exclude" },
            preview with { ActorId = "automation-1" },
            preview with { ActorKind = "automation" },
            preview with { ActorRole = "chair" },
            preview with { Rationale = "Changed rationale." },
            preview with { ExclusionReasonCode = "other-reason" },
            preview with { SupersedesDecisionDigest = "sha256:001" },
            preview with { ResolvedConflictId = "conflict-other" },
            preview with { SourceDecisionDigests = ["sha256:999"] },
            preview with { OccurredAt = FixedTime.AddMinutes(1) },
            preview with { DecisionId = "other-decision" },
            preview with { DecisionDigest = null! },
            preview with { ExpectedEffects = ["other effect"] },
            preview with { NonClaims = ["other-claim"] },
            preview with { OperationConfirmationToken = "other-operation-token" }
        ];

        foreach (var item in changed)
        {
            var result = new DesktopWorkspaceCommandFacade().ExecuteScreeningResolution(item);
            Assert.AreEqual(DesktopWorkspaceCommandStatus.Stale, result.Status);
            StringAssert.Contains(result.Message, "stale-confirmation-screening-resolution");
        }
    }

    [TestMethod]
    public void Screening_handoff_confirmation_token_binds_every_authority_and_target_field()
    {
        var preview = new DesktopScreeningHandoffPreview(
            ...
            "sha256:" + new string('8', 64), "policy", "sha256:" + new string('9', 64),
            "sha256:" + new string('a', 64), "sha256:" + new string('b', 64),
            ["sha256:" + new string('e', 64)], "alice", "human", "reviewer", "All outcomes ready.",
            FixedTime, "handoff-id", "sha256:" + new string('f', 64), ["publish one immutable handoff"],
            ["human-actor-required"], "operation-token", "pending");
        preview = preview with
        {
            ConfirmationToken = DesktopWorkspaceCommandFacade.CreateScreeningHandoffConfirmationToken(preview)
        };
        DesktopScreeningHandoffPreview[] changed =
        [
            preview with { WorkspaceDirectory = "other-workspace" },
            preview with { WorkspaceId = "other-workspace-id" },
            preview with { ExpectedProjectRevision = 2 },
            ...
            preview with { PolicyDigest = null! },
            preview with { HeaderDigest = null! },
            preview with { JournalHeadDigest = "sha256:" + new string('9', 64) },
            preview with { TargetSummaryDigests = ["sha256:" + new string('8', 64)] },
            preview with { ActorId = "mallory" },
            preview with { ActorKind = "automation" },
            preview with { ActorRole = "chair" },
            preview with { Rationale = "Changed rationale." },
            preview with { OccurredAt = FixedTime.AddMinutes(1) },
            preview with { HandoffId = "handoff-other-id" },
            preview with { HandoffDigest = null! },
            preview with { ExpectedEffects = ["other effect"] },
            preview with { NonClaims = ["other-claim"] },
            preview with { OperationConfirmationToken = "other-operation-token" }
        ];
        ...
        foreach (var item in changed)
        {
            var result = new DesktopWorkspaceCommandFacade().ExecuteScreeningHandoff(item);
            Assert.AreEqual(DesktopWorkspaceCommandStatus.Stale, result.Status);
            StringAssert.Contains(result.Message, "stale-confirmation-screening-handoff");
        }
    }
