...
Current state is audit-oriented early alpha. The July hardening sequence is complete. Protected `main` contains FE-01 through FE-09 and all nine accepted FE-08 desktop slices; use `PLANS.md`, `docs/ops/BRANCH-BOARD.md`, and the accepted gate or release evidence for the current implementation boundary. A planned feature or slice is not authorized until its ADR and gate are accepted.
